...
public class UILabel : UIWidget
{
	...
	void ProcessText (bool legacyMode, bool full)
	{
		...
		if (mFinalFontSize > 0)
		{
			...
			for (var ps = mFinalFontSize; ps > 0; --ps)
			{
				...
				// Wrap the text
				var fits = NGUIText.WrapText(printedText, out mProcessedText, true, false,
...
#else
			var wordStart = s.LastIndexOfAny(new char[] {' ', '\n'}, characterIndex) + 1;
			var wordEnd = s.IndexOfAny(new char[] { ' ', '\n', ',', '.' }, characterIndex);
#endif
	...
	public int GetCharacterIndex (int currentIndex, KeyCode key)
	{
		if (isValid)
		{
			...
			if (mTempVerts.size > 0)
			{
				...
				for (var i = 0; i < mTempIndices.size; ++i)
				{
	...
	public void PrintOverlay (int start, int end, UIGeometry caret, UIGeometry highlight, Color caretColor, Color highlightColor)
	{
		...
		var startingCaretVerts = caret.verts.size;
		...
		{
			var startingVertices = highlight.verts.size;
			...
			if (highlight.verts.size > startingVertices)
			{
				...
				for (var i = startingVertices; i < highlight.verts.size; ++i)
				{
		...
		for (var i = startingCaretVerts; i < caret.verts.size; ++i)
		{
...
#else
		for (var i = start; i < verts.size; ++i)
		{
			verts.buffer[i].x += fx;
			verts.buffer[i].y += fy;
		}
	...
	public void ApplyShadow (BetterList<Vector3> verts, BetterList<Vector2> uvs, BetterList<Color> cols, int start, int end, float x, float y)
	{
		...
		for (var i = start; i < end; ++i)
		{
			...
			var v = verts.buffer[i];
			v.x += x;
			v.y += y;
			verts.buffer[i] = v;

			var uc = cols.buffer[i];
