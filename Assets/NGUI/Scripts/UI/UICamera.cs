...
public class UICamera : MonoBehaviour
{
	...
	// Mouse events
	protected static MouseOrTouch[] mMouse = new MouseOrTouch[] { new MouseOrTouch(), new MouseOrTouch(), new MouseOrTouch() };
	...
	static public GameObject hoveredObject
	{
		...
		set
		{
			...
			if (mHover)
			{
				...
				if (statesDiffer)
				{
					var cam = (mHover != null) ? FindCameraForLayer(mHover.layer) : UICamera.list[0];
	...
	static public GameObject selectedObject
	{
		...
		set
		{
			...
			{
				var cam = (mSelected != null) ? FindCameraForLayer(mSelected.layer) : UICamera.list[0];
	...
	{
		for (var i = 0; i < 3; ++i) if (mMouse[i].pressed == go) return true;
		for (int i = 0, imax = activeTouches.Count; i < imax; ++i)
		{
			var touch = activeTouches[i];
		...
		for (var i = 0; i < list.size; ++i)
		{
			...
			if (cam.eventType == EventType.World_3D)
			{
				if (Physics.Raycast(ray, out lastHit, dist, mask))
				{
					...
					{
						var rb = FindRootRigidbody(mRayHitObject.transform);
			...
			else if (cam.eventType == EventType.UI_2D)
			{
				if (m2DPlane.Raycast(ray, out dist))
				{
					...
					var hits = Physics2D.OverlapPointAll(lastWorldPosition, mask);

					if (hits.Length > 1)
					{
						for (var b = 0; b < hits.Length; ++b)
						{
							var go = hits[b].gameObject;
							Profiler.BeginSample("Editor-only GC allocation (GetComponent)");
							var w = go.GetComponent<UIWidget>();
							Profiler.EndSample();
	...
	public virtual void ProcessTouch (bool pressed, bool released)
	{
