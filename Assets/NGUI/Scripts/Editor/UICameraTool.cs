...
public class UICameraTool : EditorWindow
{
	...
	void OnGUI ()
	{
		...
		var list = NGUIEditorTools.FindAll<Camera>();
		...
		if (list.Count > 0)
		{
			...
			foreach (var cam in list) DrawRow(cam);
	...
	{
		var highlight = (cam == null || Selection.activeGameObject == null) ? false :
		...
		var enabled = (cam == null || (NGUITools.GetActive(cam.gameObject) && cam.enabled));
		...
		{
			var uic = cam.GetComponent<UICamera>();
			var ev = (uic != null && uic.enabled);
		...
		{
			var mask = LayerMaskField(cam.cullingMask, GUILayout.Width(105f));
