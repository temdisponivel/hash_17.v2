...
public class ComponentSelector : ScriptableWizard
{
	...
	static public void Draw<T> (string buttonName, T obj, OnSelectionCallback cb, bool editButton, params GUILayoutOption[] options) where T : Object
	{
		...
		var show = NGUIEditorTools.DrawPrefixButton(buttonName);
		var o = EditorGUILayout.ObjectField(obj, typeof(T), false, options) as T;

		if (editButton && o != null && o is MonoBehaviour)
		{
			var mb = o as Component;
	...
	{
		var type = typeof(T);
		var title = (type == typeof(UIAtlas) ? "Select an " : "Select a ") + GetName(type);
		...
		else
		{
			...
			{
				for (var i = 0; i < comp.mObjects.Length; ++i)
				{
	...
	void Search ()
	{
		...
		if (mExtensions != null)
		{
			...
			for (var i = 0; i < paths.Length; ++i)
			{
				...
				for (var b = 0; b < mExtensions.Length; ++b)
				{
	...
	void OnGUI ()
	{
		...
		if (!mSearched)
		{
			...
			GUILayout.BeginHorizontal();
			GUILayout.FlexibleSpace();
			var search = GUILayout.Button("Show All", "LargeButton", GUILayout.Width(120f));
			GUILayout.FlexibleSpace();
			GUILayout.EndHorizontal();
			if (search) Search();
		}
