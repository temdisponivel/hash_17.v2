...
public static class NGUIContextMenu
{
	...
	{
		var func = obj as AddFunc;
		var widget = func(Selection.activeGameObject);
	...
	{
		var func = obj as AddFunc;
		var widget = func(Selection.activeTransform.parent.gameObject);
		if (widget != null) Selection.activeGameObject = widget.gameObject;
	}
	...
	static public void AddCommonItems (GameObject target)
	{
		if (target != null)
		{
			...
			var myName = string.Format("Selected {0}", (widget != null) ? NGUITools.GetTypeName(widget) : "Object");
			...
			var header = false;
			var scrollView = NGUITools.FindInParents<UIScrollView>(target);
...
#endif
			{
				...
				if (target.GetComponent<UIDragScrollView>() == null)
				{
					...
					{
						var pan = UIPanel.list[i];
						if (pan.clipping == UIDrawCall.Clipping.None) continue;

						var dr = pan.GetComponent<UIScrollView>();
						if (dr == null) continue;
	...
	static void AddPanel (object obj)
	{
		...
		var panel = NGUISettings.AddPanel(go);
	...
	static public void AddHelp (GameObject go, bool addSeparator)
	{
		...
		for (var i = 0; i < comps.Length; ++i)
		{
			...
			var url = NGUIHelp.GetHelpURL(type);
