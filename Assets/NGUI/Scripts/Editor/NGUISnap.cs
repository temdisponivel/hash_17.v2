...
public static class NGUISnap
{
	...
	{
		var w = child.GetComponent<UIWidget>();
		if (w != null) Add(root, child, w.localCorners);

		var p = child.GetComponent<UIPanel>();
	...
	static public Vector3 Snap (Vector3 pos, Vector3[] local, bool snapToEdges)
	{
		...
		{
			var center = pos + (local[0] + local[2]) * 0.5f;
			var bl = pos + local[0];
			var tr = pos + local[2];
			var best = new Vector2(float.MaxValue, float.MaxValue);

			for (var i = 0; i < mSnapCenter.size; ++i)
				ChooseBest(ref best, mSnapCenter[i] - center);

			for (var i = 0; i < mSnapCorners.size; ++i)
			{
