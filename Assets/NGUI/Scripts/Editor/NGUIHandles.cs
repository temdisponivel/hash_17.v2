...
public static class NGUIHandles
{
	...
	{
		float dist;
		var ray = HandleUtility.GUIPointToWorldRay(screenPos);
	...
	{
		var p = new Plane(corners[0], corners[1], corners[3]);
	...
	{
		var screenPos = new Vector2[4];

		for (var i = 0; i < 4; ++i)
			screenPos[i] = HandleUtility.WorldToGUIPoint(worldPos[i]);

		var up = (screenPos[1] - screenPos[0]).normalized;
		var right = (screenPos[3] - screenPos[0]).normalized;

		var v0 = screenPos[0] - right * 20f;
		var v1 = screenPos[1] - right * 20f;

		var v2 = screenPos[0] - up * 20f;
		var v3 = screenPos[3] - up * 20f;

		var p = new Plane(worldPos[0], worldPos[1], worldPos[3]);
	...
	{
		var s0 = HandleUtility.WorldToGUIPoint(worldPos0);
		var s1 = HandleUtility.WorldToGUIPoint(worldPos1);
	...
	{
		var screenPoint = HandleUtility.WorldToGUIPoint(worldPos);
	...
	static public void DrawCenteredLabel (Vector2 screenPos, string text, float width)
	{
		if (Event.current.type == EventType.Repaint)
		{
			...
			var tw = Mathf.Max(2, text.Length) * 15f;
