...
public class UI2DSpriteEditor : UIBasicSpriteEditor
{
	...
	{
		GUI.changed = false;
		var sp = NGUIEditorTools.DrawProperty("2D Sprite", serializedObject, "mSprite");
		...
		if (GUI.changed)
		{
			...
			if (sprite != null)
			{
				var border = serializedObject.FindProperty("mBorder");
		...
		var fa = serializedObject.FindProperty("mFixedAspect");
		var before = fa.boolValue;
	...
	public override void OnPreviewGUI (Rect rect, GUIStyle background)
	{
		...
		{
			var tex = mSprite.mainTexture as Texture2D;
