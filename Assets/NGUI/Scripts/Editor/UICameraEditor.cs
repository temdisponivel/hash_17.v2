...
public class UICameraEditor : Editor
{
	...
	{
		var cam = target as UICamera;
		GUILayout.Space(3f);

		serializedObject.Update();

		var et = serializedObject.FindProperty("eventType");
		...
		{
			var options = new string[] { "3D World", "3D UI", "2D World", "2D UI" };
			var val = EditorGUILayout.Popup("Event Type", et.intValue, options);
			if (val != et.intValue) et.intValue = val;
		}

		var ev = serializedObject.FindProperty("eventsGoToColliders");

		if (ev != null)
		{
			var val = ev.boolValue;
			var newVal = EventsGo.Colliders == (EventsGo)EditorGUILayout.EnumPopup("Events go to...",
		...
		else
		{
			...
			var mouse = serializedObject.FindProperty("useMouse");
			var touch = serializedObject.FindProperty("useTouch");
			var keyboard = serializedObject.FindProperty("useKeyboard");
			var controller = serializedObject.FindProperty("useController");
			...
			GUILayout.BeginHorizontal();
			var rd = serializedObject.FindProperty("rangeDistance");
