...
public class UIWidgetInspector : UIRectEditor
{
	...
	static public bool Raycast (Vector3[] corners, out Vector3 hit)
	{
		...
		var dist = 0f;
		var isHit = plane.Raycast(ray, out dist);
	...
	static public float GetScreenDistance (Vector3[] worldPoints, Vector2 mousePos, out int index)
	{
		...
		{
			var distance = GetScreenDistance(worldPoints[i], mousePos);
	...
	static public UIWidget.Pivot GetPivotUnderMouse (Vector3[] worldPos, Event e, bool[] resizable, bool movable, ref Action action)
	{
		...
		var pivotUnderMouse = UIWidget.Pivot.Center;

		if (action == Action.None)
		{
			var index = 0;
			var dist = GetScreenDistance(worldPos, e.mousePosition, out index);
		...
		for (var i = 0; i < 8; ++i) screenPos[i] = HandleUtility.WorldToGUIPoint(worldPos[i]);

		var b = new Bounds(screenPos[0], Vector3.zero);
		for (var i = 1; i < 8; ++i) b.Encapsulate(screenPos[i]);
	...
	public void OnSceneGUI ()
	{
		...
		var handles = GetHandles(mWidget.worldCorners);
		...
		var canResize = (mWidget.GetComponent<UIStretch>() == null);
						...
						if (Raycast(handles, out pos))
						{
							...
							if (mAction != Action.None)
							{
								...
								if (mAction == Action.Move)
								{
									...
									if (parent != null)
									{
										var grid = parent.GetComponent<UIGrid>();
									...
									var localDelta = (after - mLocalPos);
								...
								else if (mAction == Action.Scale)
								{
									...
									// Calculate the final delta
									var localDelta = (t.localPosition - mLocalPos);
	...
	static public void DrawColor (SerializedObject so, UIWidget w)
	{
		...
		{
			GUI.changed = false;
			var alpha = EditorGUILayout.Slider("Alpha", w.alpha, 0f, 1f);
	...
	static public void DrawInspectorProperties (SerializedObject so, UIWidget w, bool drawColor)
	{
		...
		var type = PrefabUtility.GetPrefabType(w.gameObject);
	...
	static void DrawDimensions (SerializedObject so, UIWidget w, bool isPrefab)
	{
		GUILayout.BeginHorizontal();
		{
			...
			{
				GUI.changed = false;
				var val = EditorGUILayout.IntField("Size", w.width, GUILayout.MinWidth(100f));
			...
			{
				GUI.changed = false;
				var val = EditorGUILayout.IntField("x", w.height, GUILayout.MinWidth(30f));
	...
	{
		var horizontal = GetHorizontal(w.pivot);
		var vertical = GetVertical(w.pivot);
