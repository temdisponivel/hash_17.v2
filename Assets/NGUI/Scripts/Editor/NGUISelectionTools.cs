...
public class NGUISelectionTools
{
	...
	{
		var gos = Selection.GetFiltered(typeof(GameObject), SelectionMode.TopLevel);
	...
	static void ActivateDeactivate()
	{
		if (HasValidTransform())
		{
			...
			var val = !NGUITools.GetActive(Selection.activeGameObject);
			foreach (var go in gos) NGUITools.SetActive(go, val);
		}
	}
...
#region Helper Functions
	...
	{
		var deps = reverse ? EditorUtility.CollectDeepHierarchy(objects) : EditorUtility.CollectDependencies(objects);
	...
	{
		var dependencies = GetDependencyList(objects, reverse);
		var list = new List<string>();
		var text = "";

		foreach (var ae in dependencies)
		{
			...
			if (ae.types.Count > 1)
			{
				...
				for (var i = 1; i < ae.types.Count; ++i)
				{
