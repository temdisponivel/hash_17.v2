...
#region Generic Get and Set methods
	...
	static public T Get<T> (string name, T defaultValue) where T : Object
	{
		...
		var retVal = NGUIEditorTools.LoadAsset<T>(path);
...
#region Convenience accessor properties
	...
	static public string pathToFreeType
	{
		get
		{
			var s = GetString(System.IntPtr.Size == 8 ? "NGUI FreeType64" : "NGUI FreeType", null);
	...
	{
		var w = NGUITools.AddWidget<UIWidget>(go);
		w.name = "Container";
		w.pivot = pivot;
		w.width = 100;
		w.height = 100;
	...
	{
		var w = NGUITools.AddWidget<UITexture>(go);
		w.name = "Texture";
		w.pivot = pivot;
	...
	{
		var w = NGUITools.AddWidget<UI2DSprite>(go);
		w.name = "2D Sprite";
		w.pivot = pivot;
		w.sprite2D = sprite2D;
		w.width = 100;
	...
	{
		var w = NGUITools.AddWidget<UISprite>(go);
		w.name = "Sprite";
		w.atlas = atlas;
		...
		{
			var sp = w.atlas.GetSprite(w.spriteName);
	...
	{
		var w = NGUITools.AddWidget<UILabel>(go);
		w.name = "Label";
	...
	static public UIPanel AddPanel (GameObject go)
	{
		...
		var depth = UIPanel.nextUnusedDepth;
		var panel = NGUITools.AddChild<UIPanel>(go);
		panel.depth = depth;
		return panel;
	}
	...
	static void PasteLabel (UILabel lbl, bool fully)
	{
		if (fully)
		{
			var obj = NGUIEditorTools.GUIDToObject(GetString("Font", null));
		...
		var x = GetFloat("Effect X", lbl.effectDistance.x);
		var y = GetFloat("Effect Y", lbl.effectDistance.y);
