...
static public class FreeType
{
	...
	static public bool isPresent
	{
		get
		{
			...
			if (!mFound)
			{
				...
				{
					var path = NGUISettings.pathToFreeType;
					mFound = File.Exists(path);
					if (mFound) LoadLibrary(path);
				}
				else
				{
					var filename = "FreeType.dylib";
	...
	static public string[] GetFaces (Font font)
	{
		...
		if (File.Exists(fileName))
		{
			...
			{
				var record = (FT_FaceRec)Marshal.PtrToStructure(face, typeof(FT_FaceRec));
				names = new string[record.num_faces];

				for (var i = 0; i < record.num_faces; i++)
				{
					var ptr = IntPtr.Zero;

					if (FT_New_Face(lib, fileName, i, out ptr) == 0)
					{
						var family = Marshal.PtrToStringAnsi(record.family_name);
						var style = Marshal.PtrToStringAnsi(record.style_name);
	...
	static public bool CreateFont (Font ttf, int size, int faceIndex, bool kerning, string characters, int padding, out BMFont font, out Texture2D tex)
	{
		...
		else
		{
			...
			foreach (var ch in characters)
			{
				...
				{
					var buffer = new byte[glyph.bitmap.width * glyph.bitmap.rows];
					Marshal.Copy(glyph.bitmap.buffer, buffer, 0, buffer.Length);

					var texture = new Texture2D(glyph.bitmap.width, glyph.bitmap.rows, UnityEngine.TextureFormat.ARGB32, false);
					var colors = new Color32[buffer.Length];

					for (int i = 0, y = 0; y < glyph.bitmap.rows; ++y)
					{
						for (var x = 0; x < glyph.bitmap.width; ++x)
						{
							white.a = buffer[i++];
			...
			var rects = tex.PackTextures(textures.ToArray(), padding);
