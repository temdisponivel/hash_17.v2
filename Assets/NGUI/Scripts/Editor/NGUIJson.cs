...
public class NGUIJson
{
	...
	static public void LoadSpriteData (UIAtlas atlas, TextAsset asset)
	{
		...
		var decodedHash = jsonDecode(jsonString) as Hashtable;
	...
	static public void LoadSpriteData (UIAtlas atlas, string jsonData)
	{
		...
		var decodedHash = jsonDecode(jsonData) as Hashtable;
	...
	static void LoadSpriteData (UIAtlas atlas, Hashtable decodedHash)
	{
		...
		var oldSprites = atlas.spriteList;
		...
		{
			var newSprite = new UISpriteData();
			...
			var table = (Hashtable)item.Value;
			var frame = (Hashtable)table["frame"];
			...
			var sourceSize = (Hashtable)table["sourceSize"];
			var spriteSize = (Hashtable)table["spriteSourceSize"];
			...
			if (spriteSize != null && sourceSize != null)
			{
				...
				if (frameW > 0)
				{
					var spriteX = int.Parse(spriteSize["x"].ToString());
					var spriteW = int.Parse(spriteSize["w"].ToString());
					var sourceW = int.Parse(sourceSize["w"].ToString());
				...
				if (frameH > 0)
				{
					var spriteY = int.Parse(spriteSize["y"].ToString());
					var spriteH = int.Parse(spriteSize["h"].ToString());
					var sourceH = int.Parse(sourceSize["h"].ToString());
	...
	public static object jsonDecode( string json )
	{
		...
		if( json != null )
		{
			...
			var index = 0;
			var success = true;
			var value = NGUIJson.parseValue( charArray, ref index, ref success );
	...
	public static string getLastErrorSnippet()
	{
		...
		{
			var startIndex = NGUIJson.lastErrorIndex - 5;
			var endIndex = NGUIJson.lastErrorIndex + 15;
			if( startIndex < 0 )
				startIndex = 0;
