...
public class UIPanelTool : EditorWindow
{
	...
	static int Compare (Entry a, Entry b)
	{
		if (a != b && a != null && b != null)
		{
			...
			if (a.panel.depth > b.panel.depth) return 1;
			var val = string.Compare(a.panel.name, b.panel.name);
			if (val != 0) return val;
	...
	{
		var panels = NGUIEditorTools.FindAll<UIPanel>();

		for (var i = panels.Count; i > 0; )
		{
	...
	void OnGUI ()
	{
		...
		if (panels != null && panels.Count > 0)
		{
			...
			var showAll = DrawRow(null, null, allEnabled);
	...
	bool DrawRow (Entry ent, UIPanel selected, bool isChecked)
	{
		...
		{
			var val = ent.panel.widgetsAreStatic;
