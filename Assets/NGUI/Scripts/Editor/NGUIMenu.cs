...
#region Create
	...
	{
		var go = NGUIEditorTools.SelectedRoot(true);
	...
	{
		var panel = NGUISettings.AddPanel(SelectedRoot());
	...
	{
		var t = NGUITools.AddChild<T>(SelectedRoot());
...
#region Options
	...
	{
		var colliders = NGUITools.FindActive<BoxCollider>();
		...
		for (var i = 0; i < colliders.Length; ++i)
		{
			...
			var p = NGUITools.FindInParents<UIPanel>(go);
	...
	{
		var colliders = NGUITools.FindActive<BoxCollider2D>();
	...
	{
		var go = Selection.activeGameObject ?? UICamera.list[0].gameObject;
