...
public static class NGUIEditorTools
{
	...
	static Texture2D CreateCheckerTex (Color c0, Color c1)
	{
		...
		for (var y = 0; y < 8; ++y) for (var x = 0; x < 8; ++x) tex.SetPixel(x, y, c1);
		for (var y = 8; y < 16; ++y) for (var x = 0; x < 8; ++x) tex.SetPixel(x, y, c0);
		for (var y = 0; y < 8; ++y) for (var x = 8; x < 16; ++x) tex.SetPixel(x, y, c0);
		for (var y = 8; y < 16; ++y) for (var x = 8; x < 16; ++x) tex.SetPixel(x, y, c1);
	...
	{
		var tex = new Texture2D(1, 16);
		tex.name = "[Generated] Gradient Texture";
		tex.hideFlags = HideFlags.DontSave;

		var c0 = new Color(1f, 1f, 1f, 0f);
		var c1 = new Color(1f, 1f, 1f, 0.4f);

		for (var i = 0; i < 16; ++i)
		{
			var f = Mathf.Abs((i / 15f) * 2f - 1f);
			f *= f;
	...
	static public void DrawOutline (Rect rect, Rect relative, Color color)
	{
		if (Event.current.type == EventType.Repaint)
		{
			...
			var x = rect.xMin + rect.width * relative.xMin;
			var y = rect.yMax - rect.height * relative.yMin;
			var width = rect.width * relative.width;
			var height = -rect.height * relative.height;
	...
	static public void DrawOutline (Rect rect, Rect outer, Rect inner)
	{
		if (Event.current.type == EventType.Repaint)
		{
			var green = new Color(0.4f, 1f, 0f, 1f);
	...
	static public GameObject SelectedRoot (bool createIfMissing)
	{
		...
		// Try to find a panel
		var p = (go != null) ? NGUITools.FindInParents<UIPanel>(go) : null;
		...
		if (createIfMissing && go == null)
		{
			...
			if (go == null)
			{
				var panel = GameObject.FindObjectOfType(typeof(UIPanel)) as UIPanel;
	...
	static public Rect IntRect (string prefix, Rect rect)
	{
		...
		var top		= Mathf.RoundToInt(rect.yMin);
		var width	= Mathf.RoundToInt(rect.width);
		var height	= Mathf.RoundToInt(rect.height);

		var a = NGUIEditorTools.IntPair(prefix, "Left", "Top", left, top);
		var b = NGUIEditorTools.IntPair(null, "Width", "Height", width, height);
	...
	{
		var left	= Mathf.RoundToInt(v.x);
		var top		= Mathf.RoundToInt(v.y);
		var right	= Mathf.RoundToInt(v.z);
		var bottom	= Mathf.RoundToInt(v.w);

		var a = NGUIEditorTools.IntPair(prefix, "Left", "Top", left, top);
		var b = NGUIEditorTools.IntPair(null, "Right", "Bottom", right, bottom);
		...
		int x, int y, int width, int height, int borderLeft, int borderBottom, int borderRight, int borderTop)
	{
		...
		if (width > 0)
		{
			var f = drawRect.width / outerRect.width;
			outerRect.width *= f;
			outerRect.height *= f;
		}
		...
		{
			var f = drawRect.height / outerRect.height;
			outerRect.width *= f;
			outerRect.height *= f;
		}
		...
		if (Selection.activeGameObject == null || Selection.gameObjects.Length == 1)
		{
			...
			// Sprite size label
			var text = string.Format("Sprite Size: {0}x{1}", Mathf.RoundToInt(width), Mathf.RoundToInt(height));
	...
	{
		var w = Mathf.RoundToInt(tex.width * uv.width);
		var h = Mathf.RoundToInt(tex.height * uv.height);
		...
		{
			var f = rect.width / outerRect.width;
			outerRect.width *= f;
			outerRect.height *= f;
		}
		...
		{
			var f = rect.height / outerRect.height;
			outerRect.width *= f;
			outerRect.height *= f;
		}
		...
		// Sprite size label
		var text = string.Format("Texture Size: {0}x{1}", w, h);
		EditorGUI.DropShadowLabel(GUILayoutUtility.GetRect(Screen.width, 18f), text);
	}
	...
	static public void DrawSpriteField (string label, SerializedObject ob, SerializedProperty atlas, SerializedProperty sprite, float width, bool padded, bool removable, params GUILayoutOption[] options)
	{
		if (atlas != null && atlas.objectReferenceValue != null)
		{
			...
			{
				var spriteName = sprite.hasMultipleDifferentValues ? "-" : sprite.stringValue;
	...
	static public List<UIWidget> SceneViewRaycast (Vector2 mousePos)
	{
		...
		for (var i = 0; i < UIPanel.list.Count; ++i)
		{
	...
	{
		var widgets = NGUIEditorTools.SceneViewRaycast(screenPos);
		var containers = new List<UIWidgetContainer>();
		...
		for (var i = 0; i < widgets.Count; ++i)
		{
			...
			var wc = NGUITools.FindInParents<UIWidgetContainer>(w.cachedGameObject);
			...
			var name = (i + 1 == widgets.Count) ? (w.name + " (top-most)") : w.name;
	...
	static public Object GUIDToObject (string guid)
	{
		...
		var id = (int)s_GetInstanceIDFromGUID.Invoke(null, new object[] { guid });
	...
	static public Color32[] AddBorder (Color32[] colors, int width, int height)
	{
		...
		{
			var y1 = NGUIMath.ClampIndex(y2 - 1, height);

			for (var x2 = 0; x2 < w2; ++x2)
			{
				var x1 = NGUIMath.ClampIndex(x2 - 1, width);
				var i2 = x2 + y2 * w2;
	...
	static public void AddShadow (Color32[] colors, int width, int height, Color shadow)
	{
		...
		for (var y2 = 0; y2 < height; ++y2)
		{
			for (var x2 = 0; x2 < width; ++x2)
			{
				...
				var c = Color.Lerp(original, sh, shadow.a * val);
