...
public class UIInputEditor : UIWidgetContainerEditor
{
	public override void OnInspectorGUI ()
	{
		...
		var label = NGUIEditorTools.DrawProperty(serializedObject, "label");
		...
		EditorGUI.BeginDisabledGroup(label == null || label.objectReferenceValue == null);
		{
			...
			EditorGUI.BeginDisabledGroup(serializedObject.isEditingMultipleObjects);
			{
				...
				{
					var ob = new SerializedObject(label.objectReferenceValue);
					ob.Update();
			...
			var sp = serializedObject.FindProperty("characterLimit");
