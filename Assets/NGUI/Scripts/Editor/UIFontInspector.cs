...
public class UIFontInspector : Editor
{
	...
	public override void OnInspectorGUI ()
	{
		...
		else if (mType == FontType.Dynamic)
		{
...
#else
			var fnt = EditorGUILayout.ObjectField("TTF Font", mFont.dynamicFont, typeof(Font), false) as Font;
			...
			var style = (FontStyle)EditorGUILayout.EnumPopup(mFont.dynamicFontStyle);
			NGUIEditorTools.DrawPadding();
			GUILayout.EndHorizontal();
		...
		else
		{
			...
			if (mFont.bmFont.isValid)
			{
				var tex = mFont.texture;

				if (tex != null && mFont.atlas == null)
				{
					// Pixels are easier to work with than UVs
					var pixels = NGUIMath.ConvertToPixels(mFont.uvRect, tex.width, tex.height, false);
					...
					var uvRect = NGUIMath.ConvertToTexCoords(pixels, tex.width, tex.height);
		...
		if (!mFont.isDynamic && mFont.bmFont.isValid)
		{
			if (mFont.atlas != null)
			{
				if (NGUIEditorTools.DrawHeader("Symbols and Emoticons"))
				{
					...
					var isValid = !string.IsNullOrEmpty(mSymbolSequence) && !string.IsNullOrEmpty(mSymbolSprite);
		...
		if (mFont.bmFont != null && mFont.bmFont.isValid)
		{
			if (NGUIEditorTools.DrawHeader("Modify"))
			{
				...
				var disable = (sd != null && (sd.paddingLeft != 0 || sd.paddingBottom != 0));
	...
	void ApplyEffect (Effect effect, Color foreground, Color background)
	{
		...
		if (mFont.atlas != null)
		{
			var sd = mFont.atlas.GetSprite(bf.spriteName);
			if (sd == null) return;
			offsetX = sd.x;
		...
		for (int i = 0, imax = glyphs.Count; i < imax; ++i)
		{
			...
			clear.a = 0;
			for (var b = 0; b < size; ++b) colors[b] = clear;
			...
			if (effect == Effect.Crop)
			{
				for (var y = 0; y < height; ++y)
				{
					...
					{
						var fx = x + glyph.x + offsetX + 1;
						var fy = y + (mFont.texHeight - glyph.y - glyph.height) + 1;
			...
			else
			{
				for (var y = 0; y < glyph.height; ++y)
				{
					...
					{
						var fx = x + glyph.x + offsetX;
						var fy = y + (mFont.texHeight - glyph.y - glyph.height);
						if (mFont.atlas != null) fy += bfTex.height - offsetY;
		...
		else
		{
			...
			var val = NGUISettings.atlasTrimming;
		...
		// Cleanup
		for (var i = 0; i < glyphTextures.Count; ++i)
