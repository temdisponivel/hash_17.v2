...
public class UIAtlasMaker : EditorWindow
{
	...
	static bool PackTextures (Texture2D tex, List<SpriteEntry> sprites)
	{
		...
		{
			for (var i = 0; i < sprites.Count; ++i) textures[i] = sprites[i].tex;
	...
	Dictionary<string, int> GetSpriteList (List<Texture> textures)
	{
		...
		if (textures.Count > 0)
		{
			...
			foreach (var tex in textures) texNames.Add(tex.name);
			texNames.Sort();
			foreach (var tex in texNames) spriteList.Add(tex, 2);
		}

		if (NGUISettings.atlas != null)
		{
			var spriteNames = NGUISettings.atlas.GetListOfSprites();
	...
	static public List<SpriteEntry> CreateSprites (List<Texture> textures)
	{
		...
		foreach (var tex in textures)
		{
			...
			var xmin = oldTex.width;
			var xmax = 0;
			var ymin = oldTex.height;
			var ymax = 0;
			var oldWidth = oldTex.width;
			var oldHeight = oldTex.height;
				...
				else
				{
					...
					for (var y = 0; y < newHeight; ++y)
					{
						for (var x = 0; x < newWidth; ++x)
						{
							...
							var oldIndex = (ymin + y) * oldWidth + (xmin + x);
	...
	{
		foreach (var se in sprites) se.Release();
	...
	static public void ReplaceSprites (UIAtlas atlas, List<SpriteEntry> sprites)
	{
		...
		var kept = new List<UISpriteData>();
		...
		for (var i = spriteList.Count; i > 0; )
		{
			var sp = spriteList[--i];
	...
	static SpriteEntry ExtractSprite (UISpriteData es, Color32[] oldPixels, int oldWidth, int oldHeight)
	{
		...
		for (var y = 0; y < newHeight; ++y)
		{
			...
			for (var x = 0; x < newWidth; ++x)
			{
				...
				var newIndex = (newHeight - 1 - y) * newWidth + x;
				var oldIndex = (oldHeight - 1 - cy) * oldWidth + cx;
	...
	static public bool UpdateTexture (UIAtlas atlas, List<SpriteEntry> sprites)
	{
		...
		var newPath = NGUIEditorTools.GetSaveableTexturePath(atlas);
	...
	static public void UpdateAtlas (UIAtlas atlas, List<SpriteEntry> sprites)
	{
		...
		else
		{
			...
			var path = NGUIEditorTools.GetSaveableTexturePath(atlas);
	...
	void OnGUI ()
	{
		...
		if (NGUISettings.atlas != null)
		{
			var mat = NGUISettings.atlas.spriteMaterial;
			var tex = NGUISettings.atlas.texture;
		...
		else
		{
			...
			GUILayout.BeginHorizontal();
			GUILayout.Space(20f);
			var create = GUILayout.Button("Create");
			GUILayout.Space(20f);
			GUILayout.EndHorizontal();
			EditorGUI.EndDisabledGroup();
			...
			if (create)
			{
				...
				if (!string.IsNullOrEmpty(path))
				{
					...
					var matPath = path.Replace(".prefab", ".mat");
					replace = true;
					...
					var prefab = (go != null) ? go : PrefabUtility.CreateEmptyPrefab(path);

					// Create a new game object for the atlas
					var atlasName = path.Replace(".prefab", "");
		...
		if (spriteList.Count > 0)
		{
			NGUIEditorTools.DrawHeader("Sprites", true);
			{
				...
				foreach (var iter in spriteList)
				{
					...
					GUILayout.Space(-1f);
					var highlight = (UIAtlasInspector.instance != null) && (NGUISettings.selectedSprite == iter.Key);
