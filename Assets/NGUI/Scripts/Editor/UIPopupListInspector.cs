...
public class UIPopupListInspector : UIWidgetContainerEditor
{
	...
	void OnEnable ()
	{
		var bit = serializedObject.FindProperty("bitmapFont");
	...
	public override void OnInspectorGUI ()
	{
		...
		var text = "";
		foreach (var s in mList.items) text += s + "\n";

		GUILayout.Space(-14f);
		GUILayout.BeginHorizontal();
		GUILayout.Space(84f);
		var modified = EditorGUILayout.TextArea(text, GUILayout.Height(100f));
		GUILayout.EndHorizontal();
		...
		if (modified != text)
		{
			...
			mList.items.Clear();
			foreach (var s in split) mList.items.Add(s);
		...
		GUI.changed = false;
		var sel = NGUIEditorTools.DrawList("Default", mList.items.ToArray(), mList.value);
