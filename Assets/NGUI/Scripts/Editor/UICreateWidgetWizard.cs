...
public class UICreateWidgetWizard : EditorWindow
{
	...
	static string LoadString (string field) { var s = EditorPrefs.GetString(field); return (string.IsNullOrEmpty(s)) ? "" : s; }
	...
	static void Load ()
	{
		...
		var color = EditorPrefs.GetInt("NGUI Color", -1);
	...
	static public bool ShouldCreate (GameObject go, bool isValid)
	{
		...
		var retVal = GUILayout.Button("Add To", GUILayout.Width(76f));
		GUI.color = Color.white;
		var sel = EditorGUILayout.ObjectField(go, typeof(GameObject), true, GUILayout.Width(140f)) as GameObject;
	...
	{
		GUILayout.BeginHorizontal();
		var c = EditorGUILayout.ColorField("Color", mColor, GUILayout.Width(220f));
	...
	void CreateButton (GameObject go)
	{
		...
		if (ShouldCreate(go, NGUISettings.atlas != null))
		{
			var depth = NGUITools.CalculateNextDepth(go);
			go = NGUITools.AddChild(go);
			go.name = "Button";

			var bg = NGUITools.AddWidget<UISprite>(go);
			...
			{
				var lbl = NGUITools.AddWidget<UILabel>(go);
	...
	void CreateImageButton (GameObject go)
	{
		...
		if (ShouldCreate(go, NGUISettings.atlas != null))
		{
			...
			var sp = NGUISettings.atlas.GetSprite(mImage0);
			...
			// Add the scripts
			var ib	= go.AddComponent<UIImageButton>();
			ib.target			= sprite;
	...
	void CreateToggle (GameObject go)
	{
		...
		if (ShouldCreate(go, NGUISettings.atlas != null))
		{
			...
			var fg = NGUITools.AddWidget<UISprite>(go);
			fg.name = "Checkmark";
	...
	void CreateScrollBar (GameObject go)
	{
		if (NGUISettings.atlas != null)
		{
			...
			GUILayout.BeginHorizontal();
			var dir = (UIScrollBar.FillDirection)EditorGUILayout.EnumPopup("Direction", mFillDir, GUILayout.Width(200f));
			GUILayout.Space(20f);
			GUILayout.Label("Add colliders?", GUILayout.Width(90f));
			var draggable = EditorGUILayout.Toggle(mScrollCL);
			GUILayout.EndHorizontal();
		...
		if (ShouldCreate(go, NGUISettings.atlas != null))
		{
			...
			var sb = go.AddComponent<UIScrollBar>();
	...
	void CreateSlider (GameObject go, bool slider)
	{
		...
		if (ShouldCreate(go, NGUISettings.atlas != null))
		{
			...
			// Background sprite
			var bgs = NGUISettings.atlas.GetSprite(mSliderBG);
			var back = (UISprite)NGUITools.AddWidget<UISprite>(go);
			...
			// Foreground sprite
			var fgs = NGUISettings.atlas.GetSprite(mSliderFG);
			var front = NGUITools.AddWidget<UISprite>(go);
			...
			if (slider)
			{
				var tbs = NGUISettings.atlas.GetSprite(mSliderTB);
				var thb = NGUITools.AddWidget<UISprite>(go);
	...
	void CreateInput (GameObject go)
	{
		...
		if (ShouldCreate(go, NGUISettings.atlas != null && NGUISettings.ambigiousFont != null))
		{
			...
			var input = go.AddComponent<UIInput>();
			input.label = lbl;
	...
	void CreatePopup (GameObject go, bool isDropDown)
	{
		...
		if (ShouldCreate(go, NGUISettings.atlas != null && NGUISettings.ambigiousFont != null))
		{
			...
			var sphl = NGUISettings.atlas.GetSprite(mListHL);
			var spfg = NGUISettings.atlas.GetSprite(mListFG);

			var hlPadding = new Vector2(Mathf.Max(4f, sphl.paddingLeft), Mathf.Max(4f, sphl.paddingTop));
			var fgPadding = new Vector2(Mathf.Max(4f, spfg.paddingLeft), Mathf.Max(4f, spfg.paddingTop));

			// Background sprite
			var sprite = NGUITools.AddSprite(go, NGUISettings.atlas, mListFG);
			sprite.depth = depth;
			sprite.atlas = NGUISettings.atlas;
			...
			// Add the popup list
			var list = go.AddComponent<UIPopupList>();
			...
			for (var i = 0; i < 5; ++i) list.items.Add(isDropDown ? ("List Option " + i) : ("Menu Option " + i));
	...
	void OnGUI ()
	{
		...
		if (!mLoaded)
		{
			...
			Load();
#if DYNAMIC_FONT
			var font = NGUISettings.ambigiousFont;
		...
		var go = NGUIEditorTools.SelectedRoot();
		...
		else
		{
			...
			GUILayout.BeginHorizontal();
			var wt = (WidgetType)EditorGUILayout.EnumPopup("Template", mWidgetType, GUILayout.Width(200f));
			GUILayout.Space(20f);
