...
public class UIFontMaker : EditorWindow
{
	...
	void MarkAsChanged ()
	{
		var obj = (Object)NGUISettings.FMFont ?? (Object)NGUISettings.BMFont;
	...
	void OnGUI ()
	{
		var fnt = (Object)NGUISettings.FMFont ?? (Object)NGUISettings.BMFont;
		var uiFont = (fnt as UIFont);
		...
		else
		{
			...
			var ttf = EditorGUILayout.ObjectField(NGUISettings.FMFont, typeof(Font), false) as Font;
			GUILayout.EndHorizontal();
			...
			if (mType == FontType.GeneratedBitmap)
			{
				...
				{
					var filename = (Application.platform == RuntimePlatform.WindowsEditor) ? "FreeType.dll" : "FreeType.dylib";
					...
					{
						var path = EditorUtility.OpenFilePanel("Find " + filename, NGUISettings.currentPath,
				...
				else if (ttf != null)
				{
					...
					EditorGUI.BeginDisabledGroup(cm != CharacterMap.Custom);
					{
						...
						var text = NGUISettings.charsToInclude;
			...
			{
				var isBuiltIn = (ttf != null) && string.IsNullOrEmpty(UnityEditor.AssetDatabase.GetAssetPath(ttf));
...
#else
		var prefabPath = EditorUtility.SaveFilePanelInProject("Save As",
		...
		else if (create == Create.Import)
		{
			...
			else
			{
				...
				var matPath = prefabPath.Replace(".prefab", ".mat");
		...
		else if (create == Create.Bitmap)
		{
				...
				NGUISettings.charsToInclude, 1, out bmFont, out tex))
			{
				...
				{
					var texPath = prefabPath.Replace(".prefab", ".png");
					var matPath = prefabPath.Replace(".prefab", ".mat");

					var png = tex.EncodeToPNG();
					var fs = File.OpenWrite(texPath);
	...
	static public bool DrawOption (bool state, string text, params GUILayoutOption[] options)
	{
		GUILayout.BeginHorizontal();
		GUILayout.Space(10f);
		var val = GUILayout.Toggle(state, text, EditorStyles.radioButton, options);
		GUILayout.EndHorizontal();
		return val;
	}
